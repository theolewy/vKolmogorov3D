...
class TimeStepper3D(CartesianTimeStepper):
    ...
    def system_specific_perturbations(self, **kwargs):
        ...
        if 'asymmetric_perturb' in kwargs.keys() and kwargs['asymmetric_perturb'] > 0:
            ...
            array *= (1 - mag * np.sin(2 * np.pi * z / self.Lz))
            field['g'] = array[local_slice]
    ...
    def _set_basic_substitutions(self):
        ...
        self.problem.substitutions['lap(A, Ay)'] = "dx(dx(A)) + dy(Ay) + dz(dz(A))"
        self.problem.substitutions['adv(A)'] = "u * dx(A) + v * dy(A) + w * dz(A)"

        self.problem.substitutions['tr'] = "c11 + c22 + c33"
        self.problem.substitutions['tr_x'] = "dx(c11) + dx(c22) + dx(c33)"
        ...
        self.problem.substitutions['t11x'] = "(dx(c11) * ftr + c11 * ftr_x ) / W "
        self.problem.substitutions['t22x'] = "(dx(c22) * ftr + c22 * ftr_x ) / W "
        self.problem.substitutions['t33x'] = "(dx(c33) * ftr + c33 * ftr_x ) / W "
        self.problem.substitutions['t12x'] = "(dx(c12) * ftr + c12 * ftr_x ) / W "
        self.problem.substitutions['t13x'] = "(dx(c13) * ftr + c13 * ftr_x ) / W "
        self.problem.substitutions['t23x'] = "(dx(c23) * ftr + c23 * ftr_x ) / W "
    ...
    def simulate(self, T=np.infty, ifreq=200, convergence_limit=1e-4,
        ...
        while self.solver.ok and not stop:
            ...
            if 'track_TW' in kwargs.keys() and kwargs['track_TW']:
                self._track_TW(ifreq=10, ifreq_log=500)
    ...
    def _TW_data_exists(self, tag):
        
        save_file = os.path.join(self.data_root, 'TW_tracker', f"{tag}.out")

        return os.path.exists(save_file)

    def _load_TW_data(self, tag):
        save_file = os.path.join(self.data_root, 'TW_tracker', f"{tag}.out")

        with open(save_file) as file:
            csv_reader = csv.reader(file, delimiter=' ')
            for row in csv_reader:
                start_x_track, start_z_track, start_sim_time_track, x_track, z_track = float(row[0]), float(row[1]), float(row[2]), float(row[3]), float(row[4])

        return start_x_track, start_z_track, start_sim_time_track, x_track, z_track
