...
def write_driveFile(material_params, system_params, solver_params, a1, a2, Tmin, lam=''):
    ...
    f.write("    vol_tr = flow.volume_average('trace')\n")
    f.write("    vol_Tr = flow.volume_average('Trace')\n")
    f.write("    trace_metric = (vol_tr - vol_Tr) / vol_Tr \n")
    f.write("    vol_KE = flow.volume_average('KE')\n")
    f.write("    vol_KE_l = flow.volume_average('KE_base')\n")
    f.write("    KE_metric = (vol_KE - vol_KE_l) / vol_KE_l \n")
    f.write("    vol_u = flow.volume_average('|u|')\n")
    f.write("    vol_U = flow.volume_average('|U|')\n")
    f.write("    u_metric = (vol_u - vol_U) / vol_U \n")

    f.write('    if (trace_metric > %e and fchannel.solver.sim_time > %e):\n'%(a2,Tmin))
    f.write('       if rank == 0:\n')
    f.write("           file = open('is2','w')\n")
    f.write("           file.write('%e, True'%(fchannel.solver.sim_time))\n")
    f.write("       logger.info('Stop trajectory: goes to field 2')\n")
    f.write('       break\n')
    f.write('    if (trace_metric < %e and fchannel.solver.sim_time > %e):\n'%(a1,Tmin))
    f.write('       if rank == 0:\n')
    f.write("           file = open('is2','w')\n")
    f.write("           file.write('%e, False'%(fchannel.solver.sim_time))\n")
