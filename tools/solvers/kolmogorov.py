...
class TimeStepper3D(CartesianTimeStepper):
    ...
    def _plot_arrays_and_metrics(self, plot_dev, subdirectory, suffix_end):
        ...
        if on_local_device():
            ...
            f"plots_W_{self.W}_Re_{self.Re}_eps_{self.eps}_beta_{self.beta}_L_{self.L}_Lx_{self.Lx:.4g}_Lz_{self.Lz:.4g}_Nx_{self.Nx}_Ny_{self.Ny}_Nz_{self.Nz}_{suffix_end}".replace('.', ',')
        else:
            f"plots_W_{self.W}_Re_{self.Re}_eps_{self.eps}_beta_{self.beta}_L_{self.L}_Lx_{self.Lx:.4g}_Nx_{self.Nx}_Ny_{self.Ny}_{suffix_end}".replace('.', ',')
        ...
        surface_level_multiplier_list = [[0.8, 0.15], 0.6, 0.3]
